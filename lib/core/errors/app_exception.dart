...
class NetworkException extends AppException {
  const NetworkException([String message = 'No internet connection. Please check your network.', {Object? cause}])
      : super(message, cause: cause);
}

/// Thrown when the user is not authenticated or their session has expired.
class AuthException extends AppException {
  const AuthException([String message = 'Please sign in to continue.', {Object? cause}])
      : super(message, cause: cause);
}

/// Thrown when the user does not have permission to perform an operation.
class PermissionException extends AppException {
  const PermissionException([String message = 'You do not have permission to perform this action.', {Object? cause}])
      : super(message, cause: cause);
}

/// Thrown when the requested resource does not exist.
class NotFoundException extends AppException {
  const NotFoundException([String message = 'The requested item was not found.', {Object? cause}])
      : super(message, cause: cause);
}
...
class ServerException extends AppException {
  ...
  const ServerException([
    ...
    {Object? cause, this.statusCode}
  ]) : super(message, cause: cause);
}

/// Thrown when local data (Hive, SharedPreferences) cannot be read or written.
class CacheException extends AppException {
  const CacheException([String message = 'Failed to load cached data.', {Object? cause}])
      : super(message, cause: cause);
}
...
class UnknownException extends AppException {
  const UnknownException([String message = 'An unexpected error occurred.', {Object? cause}])
      : super(message, cause: cause);
}
